...
public:
	...
	connectivity(shared_ptr<master> given_origin,shared_ptr<slave> given_end,int given_type)
	:_origin(given_master),_end(given_slave),_type(given_type){}
	shared_ptr<master> &origin() {return _origin;}
	shared_ptr<slave> &end() {return _end;}
	const int &type() {return _type;}
	void define_origin(shared_ptr<master> g_master){
		_master=g_master;}
	void define_end(shared_ptr<slave> g_slave){
		_end=g_end;}
